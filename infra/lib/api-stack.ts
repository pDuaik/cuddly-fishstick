  ...
  constructor(scope: Construct, id: string, props: ApiStackProps) {
    ...
    const authCallbackFn = new NodejsFunction(this, 'AuthCallbackFn', {
      ...
      environment: {
        ...
        COGNITO_ISSUER: `https://cognito-idp.${this.region}.${this.urlSuffix}/${cognitoUserPoolId}`,
        POST_LOGIN_REDIRECT: '/app/page1.html',
        COOKIE_NAME: 'session',
    ...
    const privateEndpoints = new Set<NodejsFunction>();
    const publicEndpoints = new Set<NodejsFunction>();
     ...
     */
    const writeWrapperEntrypoint = (opts: {
      id: string;
      userEntryAbs: string;
      exportName: string;
      wrapperName: 'secureHttp' | 'publicHttp';
      wrapperModule: string;
    }): string => {
      const genDir = path.join(repoRoot, '.generated', 'user-endpoints');
      ensureDir(genDir);

      const fileSafe = `${this.sanitizeId(opts.id)}-${opts.wrapperName}-${sha1(opts.userEntryAbs)}.ts`;
      ...
      // wrapper modules are in: infra/lambda/api/*.ts
      const src = `
// AUTO-GENERATED by ApiStack. DO NOT EDIT.
import { ${opts.wrapperName} } from '${opts.wrapperModule}';
import { ${opts.exportName} as business } from ${JSON.stringify(relUser)};

export const handler = ${opts.wrapperName}(business);
`.trimStart();
    ...
    const createEndpoint = (input: CreateUserEndpointInput, opts: {
      factoryName: string;
      ...
      if (!idRaw) throw new Error(`${opts.factoryName}: "id" is required.`);
      if (!entryRelRaw) throw new Error(`${opts.factoryName}(${idRaw}): "entryRelativeToUserDir" is required.`);
      if (!/^[A-Za-z_$][A-Za-z0-9_$]*$/.test(exportName)) {
        throw new Error(`${opts.factoryName}(${idRaw}): exportName must be a JavaScript identifier.`);
      }
      ...
      if (entryRel.includes('..')) {
        throw new Error(
          `${opts.factoryName}(${idRaw}): entryRelativeToUserDir must not contain "..": ${entryRelRaw}`,
        );
      }
      ...
      for (const k of Object.keys(opts.requiredEnv)) {
        if (k in userEnv) {
          throw new Error(`${opts.factoryName}(${idRaw}): environment cannot override required var "${k}".`);
        }
      }
      ...
      for (const k of Object.keys(userEnv)) {
        if (k.startsWith(PLATFORM_ENV_PREFIX)) {
          throw new Error(
            `${opts.factoryName}(${idRaw}): environment key "${k}" is not allowed. ` +
      ...
      const wrapperEntry = writeWrapperEntrypoint({
        ...
        wrapperModule: opts.wrapperModule,
      });


      // Prefix to reduce collisions
      const safeId = this.sanitizeId(idRaw);
      const constructId = `${opts.constructPrefix}${safeId}`;
      ...
      (opts.wrapperName === 'secureHttp' ? privateEndpoints : publicEndpoints).add(fn);
      return fn;
    };

    const createUserEndpoint: UserExtensionCtx['endpoint']['createUserEndpoint'] = (input) => createEndpoint(input, {
      factoryName: 'createUserEndpoint',
      constructPrefix: 'UserEndpoint',
      requiredEnv: requiredUserEnv,
      wrapperName: 'secureHttp',
      wrapperModule: '../../lambda/api/secure-http',
    });

    const createPublicEndpoint: UserExtensionCtx['publicEndpoint']['createPublicEndpoint'] = (input) => createEndpoint(input, {
      factoryName: 'createPublicEndpoint',
      constructPrefix: 'PublicUserEndpoint',
      requiredEnv: requiredPublicUserEnv,
      wrapperName: 'publicHttp',
      wrapperModule: '../../lambda/api/public-http',
    });

    const allowedMethods = new Set<string>(['GET', 'POST', 'PUT', 'PATCH', 'DELETE', 'OPTIONS', 'HEAD']);
    const allowedPublicMethods = new Set<string>(['GET', 'OPTIONS', 'HEAD']);
    ...
    const normalizePublicMethod = (m: PublicUserApiMethod): apigwv2.HttpMethod => {
      const s = String(m ?? '').trim().toUpperCase();
      if (!allowedPublicMethods.has(s)) throw new Error(`registerPublicApiRoute: method not allowed: ${s}`);
      return s as apigwv2.HttpMethod;
    };
    ...
    const registerApiRoute: UserExtensionCtx['api']['registerApiRoute'] = (input) => {
      ...
      if (routePath.startsWith('/api/public/')) {
        throw new Error(`registerApiRoute: "/api/public/*" is reserved for registerPublicApiRoute: ${routePath}`);
      }
      ...
      if (!privateEndpoints.has(fn)) {
        throw new Error(`registerApiRoute(${routePath}): fn must come from this platform's createUserEndpoint().`);
      }
    ...
    const registerPublicApiRoute: UserExtensionCtx['publicApi']['registerPublicApiRoute'] = (input) => {
      const routePath = (input?.path ?? '').toString().trim();

      if (!routePath.startsWith('/api/public/')) {
        throw new Error(`registerPublicApiRoute: path must start with "/api/public/": ${routePath}`);
      }

      const methodsRaw = input?.methods ?? [];
      if (!Array.isArray(methodsRaw) || methodsRaw.length === 0) {
        throw new Error(`registerPublicApiRoute(${routePath}): methods must be a non-empty array.`);
      }

      const fn = input?.fn;
      if (!fn) throw new Error(`registerPublicApiRoute(${routePath}): "fn" is required.`);
      if (!publicEndpoints.has(fn)) {
        throw new Error(`registerPublicApiRoute(${routePath}): fn must come from this platform's createPublicEndpoint().`);
      }

      const methods = methodsRaw.map(normalizePublicMethod);

      const integrationId =
        `PublicUserIntegration${this.sanitizeId(routePath)}${methods.join('')}${this.sanitizeId(fn.node.id)}`;
      const integration = new apigwv2Integrations.HttpLambdaIntegration(integrationId, fn);
    ...
    const ctx: UserExtensionCtx = {
      ...
      publicApi: { registerPublicApiRoute },
    };
